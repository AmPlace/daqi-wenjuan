...
## 项目启动
建议和仓库内的 mock 服务一起运行。当前 mock 服务内置了一份固定的“大漆非遗文创产品市场调研调查问卷”（40 题、1533 份可重复答卷），不会在刷新时随机改变。
```bash
# 安装依赖
npm install

# 终端一：启动 API mock（监听 3001）
npm run mock
...
启动后打开 `http://localhost:8000/manage/list`，点击问卷标题可进入统计页；固定问卷 ID 为 `daqi-heritage-market-2026`。统计页默认展示 40 道题的汇总统计，可切换到“单题分析”查看图表和详细答卷。
如果 8000 端口已被占用，开发服务器会自动询问并切换到 8001。

该 mock 是固定数据的本地展示服务：查询、分页和统计可用，创建、复制、编辑、标星和删除请求不会持久化，刷新后会恢复上述固定问卷和 1533 份答卷。
